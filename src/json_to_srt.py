...
# def getPhrasesFromTranscriptSegments( transcript ):

# 	ts = json.loads( transcript )
# 	segments = ts['results']['audio_segments']
...
def getPhrasesFromTranscript( transcript ):
	...
	for index, item in enumerate(items):
		...
		if index < len(items) - 1:
			next_phrase = items[index+1]["type"]
			next_phrase_len = len(items[index+1]['alternatives'][0]["content"])

		future_count = next_phrase_len + get_char_count(phrase)
  
		if future_count >= 32 and next_phrase != "punctuation":
			phrases.append(phrase)
			phrase = newPhrase()
			nPhrase = True
			x = 0
